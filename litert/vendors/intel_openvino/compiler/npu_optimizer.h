...
//
// Gemma4 (decode) exports each expert as an independent SwiGLU branch with its
...
//      computed on-device (lowered to GatherDMA by the NPU compiler — see the
//      §7 GatherDMA notes in vpux_knowleg.md).
...
class MoEGatherSelect : public ov::pass::ModelPass {
 public:
  OPENVINO_RTTI("MoEGatherSelect");
...
class NpuOptimizer {
 public:
  ...
  // Toggles the MoEGatherSelect pass. Disabled by default; enable via config
